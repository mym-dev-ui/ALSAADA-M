...
export default function RegisterPage() {
  ...
  const selectCls = (err?: string) =>
    `w-full border rounded-xl px-4 py-3.5 text-sm text-right appearance-none bg-white focus:outline-none transition-colors ${err ? "border-red-400" : "border-gray-200 focus:border-[#0c6e3e]"} text-gray-700`;
        ...
        <form onSubmit={handleSubmit} noValidate className="px-5 pb-10 space-y-4">
          ...
          <div id="f-delivery_date">
            ...
            <input type="date" value={data.delivery_date} onChange={set("delivery_date")} min={new Date().toISOString().split("T")[0]}
              className={`w-full border rounded-xl px-4 py-3.5 text-sm bg-white focus:outline-none transition-colors text-gray-700 ${errors.delivery_date ? "border-red-400" : "border-gray-200 focus:border-[#0c6e3e]"}`}
            />
