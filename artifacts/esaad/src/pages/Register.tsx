...
function validateId(v: string) {
  return /^784[-\s]?\d{4}[-\s]?\d{7}[-\s]?\d$/.test(v.trim()) || v.trim().length >= 9;
}
...
export default function Register() {
  ...
  const selectCls = (err?: string) =>
    `w-full border rounded-xl px-4 py-3.5 text-sm text-right appearance-none bg-white focus:outline-none transition-colors ${
      err ? "border-red-400" : "border-gray-200 focus:border-[#0c6e3e]"
    } ${!data.membership && data.membership === "" ? "text-gray-400" : "text-gray-700"}`;
    ...
    <div className="min-h-screen bg-gray-50" dir="rtl">
      <div className="mx-auto w-full max-w-md bg-white min-h-screen sm:shadow-xl">
        ...
        <div className="px-5 pt-5 pb-3">
          <img
            src={`${BASE}esaad-about.png`}
            alt="بطاقة إسعاد"
            className="w-full object-contain drop-shadow-md rounded-2xl max-h-52"
            loading="lazy"
          />
        </div>
        ...
        <form onSubmit={handleSubmit} noValidate className="px-5 pb-10 space-y-4">
          ...
          <div id="f-membership">
            <label className="block text-xs font-bold text-gray-600 mb-1.5">
              نوع العضوية <span className="text-red-400">*</span>
            </label>
            <div className="relative">
              <select
                value={data.membership}
                onChange={set("membership")}
                className={selectCls(errors.membership)}
              >
          ...
          <div id="f-emirate">
            <label className="block text-xs font-bold text-gray-600 mb-1.5">
              الإمارة / المدينة <span className="text-red-400">*</span>
            </label>
            <div className="relative">
              <select
                value={data.emirate}
                onChange={set("emirate")}
                className={selectCls(errors.emirate)}
              >
          ...
          <div id="f-delivery_date">
            <label className="block text-xs font-bold text-gray-600 mb-1.5">
              تاريخ التوصيل <span className="text-red-400">*</span>
            </label>
            <input
              type="date"
              ...
              className={`w-full border rounded-xl px-4 py-3.5 text-sm bg-white focus:outline-none transition-colors text-gray-700 ${
                errors.delivery_date ? "border-red-400" : "border-gray-200 focus:border-[#0c6e3e]"
              }`}
            />
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-4 mt-2 text-base font-bold rounded-2xl text-white transition-all duration-200 hover:opacity-90 active:scale-[0.98] disabled:opacity-70 flex items-center justify-center gap-2"
